...
fn spawn_tls_server(body: &'static [u8]) -> u16 {
    ...
    std::thread::spawn(move || {
        while let Ok((sock, _)) = listener.accept() {
            ...
            let body = body.clone();
            std::thread::spawn(move || serve_connection(sock, cfg, body));
        }
    });
...
#[test]
fn multi_provider_outcome_attribution() {
    let port0 = spawn_tls_server(br#"{"jsonrpc":"2.0","result":"sig1","id":1}"#);
    let port1 = spawn_tls_server(
        br#"{"jsonrpc":"2.0","error":{"code":-32002,"message":"blockhash not found"},"id":1}"#,
    );
    ...
    let sender = match Sender::builder()
        ...
        .tls(client_cfg)
        .provider(provider_for(port0))
        .provider(provider_for(port1))
    ...
    sender.trigger(Job { id: 55, ctx: 0 }).unwrap();
    ...
    outcomes.sort_by_key(|o| o.provider.0);

    assert_eq!(outcomes[0].provider.0, 0);
    assert_eq!(outcomes[0].job, 55);
    match &outcomes[0].kind {
        OutcomeKind::Accepted { signature } => assert_eq!(signature, "sig1"),
        other => panic!("{other:?}"),
    }

    assert_eq!(outcomes[1].provider.0, 1);
    assert_eq!(outcomes[1].job, 55);
